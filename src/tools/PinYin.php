...
class PinYin
{
     ...
     * @return string
     */
    public function turn($chinese, $ucFirst = false, $delimiter = '', $charset = 'utf-8') {
        $check = preg_match('/^[\-\_a-zA-Z0-9\.]+$/', $chinese, $match);
        if ($check) {
            $result = strtolower($chinese);
        } else {
        ...
        return preg_replace("/[^a-z0-9_\-\s\.]*/i", '', $result);
    }
     ...
     * @return int|string
     */
    private function turnPinYin($num, $data) {
        if ($num > 0 && $num < 160) {
            return chr($num);
